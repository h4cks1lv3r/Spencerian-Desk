...
import json, math, subprocess, tempfile, xml.etree.ElementTree as ET
import numpy as np
...
def graph_paths(skel,scale):
    ...
    for _ in range(4):
      ...
      for p in pts:
        ...
        if len(nbr[cur])>2 and len(path)<5*scale:discard.update(path)
...
def smooth_svg(points,smooth=.35):
...
def create(row):
    ...
    cutoff=.105 if row['lessonId']=='c08' else .065 if row['lessonId']=='c09' else .14
    mask=soft>cutoff
    lab,num=ndimage.label(mask);counts=np.bincount(lab.ravel());keep=counts>(12 if row['lessonId']=='c09' else 180);keep[0]=False;mask=keep[lab]
    ...
    small=(hole_sizes<(.3 if row['lessonId']=='c09' else 9)*scale*scale);small[0]=False
    ...
    for points in paths:
      d=smooth_svg(points,smooth=.35)
      if d:ET.SubElement(group,ns+'path',{'d':d})
    # Dark original swells only: smooth contours independent of faint hairlines.
    shade=ndimage.gaussian_filter(up,sigma=.8*scale)>.35
    lab,num=ndimage.label(shade);counts=np.bincount(lab.ravel());keep=counts>(9 if row["lessonId"]=="c09" else 450);keep[0]=False;shade=keep[lab]
...
if __name__=='__main__':
 for row in ROWS:
  if row['lessonId'] in ['c01','c02','c03','c04','c05','c06','c07','c08','c09','c10']:create(row)
