...
(function () {
  ...
  function installStyles(){
    ...
    const style=document.createElement('style');style.id='practice-lab-styles';style.textContent=`
      ...
      .pl-toolbar{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-bottom:14px}.pl-field{display:flex;flex-direction:column;gap:5px;flex:1;min-width:155px}.pl-label,.pl-eyebrow{font-size:12px;text-transform:uppercase;letter-spacing:.13em;color:var(--pl-muted);font-weight:700}
      ...
      .pl-paper-wrap{position:relative;background:#fffdf8;border-radius:8px;overflow:hidden;border:1px solid #d7d1c4;box-shadow:0 10px 25px rgba(49,46,38,.07),0 2px 0 #f8f6f1,0 3px 0 #dedacf}.pl-paper-wrap:after{content:'';position:absolute;top:0;right:0;width:18px;height:18px;background:linear-gradient(45deg,#e1dacc 0%,#f1ece2 49%,#d7d1c4 50%,#f8f6f1 52%);border-bottom-left-radius:3px;box-shadow:-1px 1px 3px #262b2912;pointer-events:none}.pl-paper-heading{display:flex;justify-content:space-between;align-items:center;gap:8px;padding:15px 30px 0 17px;color:#686b63;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.13em}.pl-paper-heading strong{color:#253a33;font-family:'EB Garamond',Georgia,serif;font-size:14px;font-weight:400;text-transform:none;letter-spacing:.01em}.pl-canvas{display:block;width:100%;aspect-ratio:24/13;touch-action:none;cursor:crosshair;user-select:none;-webkit-user-select:none}.pl-paper-footer{display:flex;justify-content:space-between;padding:0 17px 13px;color:#686b63;font-size:12px;letter-spacing:.03em}
      .pl-guide-key{display:flex;gap:13px;flex-wrap:wrap;color:var(--pl-muted);font-size:12px;margin:12px 0}.pl-guide-key span:before{content:'';display:inline-block;width:15px;border-top:1px solid #b4afa2;vertical-align:middle;margin-right:5px}.pl-guide-key span:nth-child(2):before{border-top-style:dashed}.pl-guide-key span:nth-child(3):before{border-top-width:2px;border-color:var(--pl-accent)}.pl-options{display:flex;flex-wrap:wrap;gap:14px;align-items:center;margin:16px 0}.pl-check{display:flex;align-items:center;gap:7px;font-size:12px;color:#454b45;cursor:pointer;min-height:28px}.pl-check input{accent-color:var(--pl-emphasis);width:16px;height:16px}.pl-range{accent-color:var(--pl-accent);width:95px}.pl-speed{font-size:12px;color:var(--pl-muted);display:flex;align-items:center;gap:7px}.pl-sheet-size{width:156px;min-height:42px;padding:9px 10px;font-size:12px}.pl-guide-description{color:var(--pl-muted);font-size:12px;line-height:1.55;margin:0 0 10px}.pl-actions{display:flex;flex-wrap:wrap;gap:8px;align-items:center}.pl-actions .pl-spacer{flex:1;min-width:0}
      .pl-metrics{display:grid;grid-template-columns:1fr 1fr 1fr;gap:9px;margin-top:17px}.pl-metric{background:#f8f6f1;border:1px solid var(--pl-line);border-radius:7px;padding:12px 13px}.pl-metric strong{display:block;font-family:'EB Garamond',Georgia,serif;font-size:21px;font-weight:400;color:var(--pl-emphasis);margin-bottom:5px}.pl-metric span{font-size:12px;color:var(--pl-muted)}.pl-note{font-size:12px;line-height:1.65;color:#454b45;margin:15px 0 0}.pl-note strong{color:#253a33;font-weight:600}.pl-method{font-size:12px;line-height:1.6;color:#686b63;margin:8px 0}.pl-pressure{display:inline-flex;align-items:center;gap:6px;border-radius:20px;background:#f1f3ed;border:1px solid #d7dfd1;color:#405047;font-size:12px;padding:7px 10px;margin-top:10px}.pl-pressure:before{content:'';width:5px;height:5px;border-radius:50%;background:var(--pl-accent)}.pl-status{font-size:12px;color:#405047;min-height:17px;margin-top:10px}.pl-opacity{display:flex;gap:7px;align-items:center;font-size:12px;color:var(--pl-muted)}
      @media(max-width:420px){.pl-btn{padding:10px 11px;font-size:12px}.pl-metrics{gap:6px}.pl-metric{padding:10px}.pl-metric strong{font-size:19px}.pl-paper-heading{font-size:12px;padding:13px 26px 0 12px}.pl-paper-heading strong{font-size:12px}.pl-paper-footer{padding:0 12px 10px;font-size:12px}.pl-options{gap:9px 13px}.pl-actions .pl-spacer{display:none}}
    `;document.head.appendChild(style);
  }
  function validDrawing(data){
    if(!data||typeof data!=='object'||Array.isArray(data)||data.version!==1||!Object.prototype.hasOwnProperty.call(templates,data.template)||!['movement','letters'].includes(data.guideMode)||![50,52].includes(data.slant)||!Number.isFinite(data.duration)||data.duration<0||data.duration>86400||!Array.isArray(data.strokes)||data.strokes.length>2000)return false;
    let points=0;
    return data.strokes.every(stroke=>stroke&&typeof stroke==='object'&&['pen','touch','mouse'].includes(stroke.pointerType)&&typeof stroke.pressureVerified==='boolean'&&Array.isArray(stroke.points)&&stroke.points.length>0&&stroke.points.length<=12000&&(points+=stroke.points.length)<=100000&&stroke.points.every(p=>p&&typeof p==='object'&&Number.isFinite(p.x)&&p.x>=0&&p.x<=1&&Number.isFinite(p.y)&&p.y>=0&&p.y<=1&&(p.p===null||Number.isFinite(p.p)&&p.p>=0&&p.p<=1)&&Number.isFinite(p.t)&&p.t>=0&&p.t<=1e12));
  }
  function mount(container, options={}){
    ...
    function getDrawing(){return{version:1,template:type,guideMode,slant,duration:Math.round(duration()),strokes:JSON.parse(JSON.stringify(current?strokes.concat(current):strokes))}}
    function checkpoint(){if(options.onDraft)options.onDraft(getDrawing())}
    ...
    function finishPointer(event,cancelled=false){if(activePointer!==event.pointerId)return;if(current){if(!cancelled){const p=pointerPoint(event),last=current.points[current.points.length-1];if(Math.hypot((p.x-last.x)*W,(p.y-last.y)*H)>.35)current.points.push(p);strokes.push(current);}current=null;}try{canvas.releasePointerCapture(activePointer);}catch(e){}activePointer=null;update();checkpoint();schedule();if(cancelled)status('Interrupted stroke discarded.');}
    function clear(){if(current)return;strokes=[];elapsed=0;startTime=0;update();checkpoint();if(options.onClear)options.onClear();schedule();status('Practice cleared.');}
    function undo(){if(current)return;strokes.pop();update();checkpoint();schedule();status('Last stroke removed.');}
    ...
    function loadDrawing(data,{trusted=false}={}){
      if(current)return false;
      // Runtime snapshots come only from getDrawing and must retain every point,
      // even when they are too large for automatic recovery. Persisted drafts are
      // validated as a whole, never silently shortened or partially accepted.
      if(!trusted&&!validDrawing(data)){status('This practice draft could not be recovered. Saved progress is unchanged.');return false;}
      strokes=data.strokes.map(s=>({pointerType:s.pointerType,pressureVerified:s.pressureVerified,points:s.points.map(p=>({x:p.x,y:p.y,p:p.p,t:p.t}))}));
      pressureVerified=strokes.some(s=>s.pressureVerified);startTime=0;elapsed=data.duration;
      setTemplate(data.template);setGuideMode(data.guideMode);setSlant(data.slant);update();schedule();status('Practice draft recovered.');return true;
    }
    ...
    resize();update();
    return {destroy(){destroyed=true;if(frame)cancelAnimationFrame(frame);if(resizeObserver)resizeObserver.disconnect();listeners.forEach(off=>off());root.remove();},clear,undo,setTemplate,setGuideMode,setSlant,getImage,getStats,getDrawing,exportSheet,loadDrawing};
  }
  ...
  window.PracticeLab={mount,templateSVG,validDrawing};
})();
