...
final class AiClient {
    ...
    static String request(JSONObject config, JSONObject request) throws Exception {
        ...
        try {
            ...
            String response;
            try (InputStream input = connection.getInputStream()) { response = readBounded(input, 1500000); }
            return parseResponse(provider, new JSONObject(response));
        } finally { connection.disconnect(); }
    }
    static String parseResponse(String provider, JSONObject response) throws Exception {
        ...
        if (value.isEmpty()) throw new UserError("The provider returned no text. Check model support or try a simpler prompt.");
        return value.length() > 50000 ? value.substring(0, 50000) + "\n[Response shortened]" : value;
    }
    static String readBounded(InputStream input, int maximum) throws Exception {
        ...
        while ((count = input.read(buffer)) != -1) { total += count; if (total > maximum) throw new UserError("The returned file is too large."); output.write(buffer, 0, count); }
